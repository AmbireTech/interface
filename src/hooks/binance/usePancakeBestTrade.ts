 ...
 */
export function usePancakeBestTrade(
  tradeType: TradeType,
  ...
  const univ2Trade = useMemo(() => {
    ...
    const currency1 = new V2Token(SupportedChainId.BINANCE, tokenA.address, tokenA.decimals, tokenA.symbol, tokenA.name)
    const currency2 = new V2Token(SupportedChainId.BINANCE, tokenB.address, tokenB.decimals, tokenB.symbol, tokenB.name)
