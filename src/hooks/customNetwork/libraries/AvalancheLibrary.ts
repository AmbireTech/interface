...
import { ChainId, Currency, CurrencyAmount, Pair, Percent, Router, Token, TokenAmount, Trade } from '@traderjoe-xyz/sdk'
...
export class AvalancheLibrary extends UniV2CustomLibrary {
  ...
  getPair(tokenA: Token, tokenB: Token, reserve0: string, reserve1: string): PairObject {
    return new Pair(
      new TokenAmount(tokenA as Token, reserve0),
      new TokenAmount(tokenB as Token, reserve1),
      ChainId.AVALANCHE
    )
  }

  getPairAddress(tokenA: TokenObject, tokenB: TokenObject): string {
    return Pair.getAddress(tokenA as Token, tokenB as Token, ChainId.AVALANCHE)
  }
  ...
  getRouterCalldata(methodName: string, args: (string | string[])[]): string {
    ...
    return JoeRouterInterface.encodeFunctionData(methodName, args)
  }

  getTradeMaxAmountIn(trade: TradeObject, slippage: V2Percent): AmountObject {
    const joeTrade = trade as Trade
    return joeTrade.maximumAmountIn(this._convertPercent(slippage))
  }

  isTradeInputToken(trade: TradeObject): boolean {
    const joeTrade = trade as Trade
    return joeTrade.inputAmount instanceof TokenAmount && Boolean(joeTrade.inputAmount.token?.address)
  }
}
